...
/// Replace NEWMODULE with the name of your module
  ...
  // Get all required strings
    ...
    $strconsult = get_string('activity', 'indicadors');
	...
	$form.="<TR>";
	$form.='<TD colspan="3">'.get_string('activity', 'indicadors').'</TD></TR>';
	...
	$form.='</SELECT>';	
	$form.='</TD></TR><TR><TD align=right>'.get_string('module','indicadors').' : &nbsp;</TD>';
	...
	$form.='<INPUT TYPE="checkbox" name="indform[each_hour]" '.$posiblechecked.'>'.get_string('do_it_for_each_hour','indicadors').'</TD></TR>';
	$form.='</TABLE><p>';
...
///// Miguel Angel Quintero
	if (isset ($indform['each_hour'])&&isset ($indform['button'])){
		...
		$table_consult->head= array (get_string('date', 'indicadors'), get_string('module','indicadors'), get_string('reads', 'indicadors'),get_string('writes','indicadors'));
		$table_consult->wrap = array ('nowrap','');
