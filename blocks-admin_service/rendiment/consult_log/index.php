...
/// Get all required strings
	...
	if($entorn){
 		if(!preparar_entorn($entorn)){
    		print_error('noconfenv','service');    
    	}
    }
    
    
    $strindicadorss = get_string("modulenameplural", "indicadors");
    $strconsult = get_string('logins', 'indicadors');
	...
	$form.="<TR>";
	$form.='<TD colspan="3">'.get_string('logats', 'indicadors').'</TD></TR>';
	$form.='<tr><td colspan="3">&nbsp;</td></tr>';
	$form.= '<tr><td align="right"><b>'.get_string('select_clients','indicadors').'</b></td>';
	
	if($connexions){
		$form.= '<td align="left"><form action="index.php" method="post">' .
			'<select name="entorn" onChange="this.parentNode.submit();">';
		$form.='<option value="-1">'.get_string('select_clients','indicadors').'</option>';
		...
		$form.= '<td align="left">'.get_string('noentornconfig','service').'</td>';
	}
	$form.='<tr><td colspan="3">&nbsp;</td></tr>';
	
	$form.='<TR><TD align=right>'.get_string('gran_all_from','indicadors').'</TD><TD align="left">';			
	...
	$form.= get_string('gran_all_with', 'indicadors');
	$form.='</TD><TD align="left">';
	$form.='<SELECT name= "indform[granul]">';
	if (isset ($indform['button'])){
		if($indform['granul']==3){
			$form.='<OPTION VALUE="1">'.get_string('day', 'indicadors').'</OPTION>';
			$form.='<OPTION VALUE="2">'.get_string('month', 'indicadors').'</OPTION>';
			//$form.='<OPTION SELECTED VALUE="3">'.get_string('year', 'indicadors').'</OPTION>';
		}else if($indform['granul']==2){
			$form.='<OPTION VALUE="1">'.get_string('day', 'indicadors').'</OPTION>';
			$form.='<OPTION SELECTED VALUE="2">'.get_string('month', 'indicadors').'</OPTION>';
			//$form.='<OPTION VALUE="3">'.get_string('year', 'indicadors').'</OPTION>';
		}else{	
			$form.='<OPTION SELECTED VALUE="1">'.get_string('day', 'indicadors').'</OPTION>';
			$form.='<OPTION VALUE="2">'.get_string('month', 'indicadors').'</OPTION>';
			//$form.='<OPTION VALUE="3">'.get_string('year', 'indicadors').'</OPTION>';			
		}	
	}else{
		...
		$results = array();
			if (isset ($indform['tot'])){
				...
				if(generic_connect()){
					...
					print_error('connectionproblem','service');
				}
				...
				if ($results){
					...
					case 1:
						...
						foreach ($resultats as $d => $result){
							$h=date('G',$d);
							//$franja = $temp[0].' '.get_string('of','indicadors').' '.$h.'-'.($h+1);
							$franja = date("Y/m/d", $d).' '.get_string('of','indicadors').' '.$h.'-'.($h+1);
					...
					case 2:$table_consult->head= array (get_string('date','indicadors'), get_string('number_backends','indicadors'), get_string('max','indicadors'), get_string('hour','indicadors'));
						...
						foreach($results as $result){
							if(!$dades = get_user_info_from_db('id',$result->userid)){
								$nom = get_string('not_found','indicadors');
							}else{
                ...
                //Matrix Data - Num
                //$head = array('date', 'num');
                //$graphResult = graph_from_array($name, $head, $rows);
                //print_object($graphResult);
				...
				$table_consult->head= array (get_string('date', 'indicadors'), get_string('times', 'indicadors'));
				$table_consult->wrap = array ('nowrap','');
