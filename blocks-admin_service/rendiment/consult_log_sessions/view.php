...
/// Replace NEWMODULE with the name of your module
  ...
  $strindicadorss = get_string("modulenameplural", "indicadors");
    $strconsult = get_string('consult_smoodle', 'indicadors');
...
//FUNCTIONS FOR THIS CONSULT
 $indform = optional_param('indform');
	...
	//$form.='<TD colspan="2">'.get_string('consult_smoodle', 'indicadors').'</TD></TR>';
	...
	//$form.='<TD colspan="2">'.get_string('log_sessions', 'indicadors').'</TD></TR>';

	
	$form.='<TR><TD align="right">'.get_string('date','indicadors').':</td><td align="left">';			
...
function print_cerca($indform,$hoy,$mes,$any){
	...
	if (isset ($indform['button'])){
		...
		$start_time->any = $indform['any_ini'];
		$start_time->mes = $mes;
		$start_time->dia = $indform['dia_ini'];
			...
			$table_consult->head= array (get_string('date', 'indicadors'), get_string('smoodle','indicadors'), get_string('spostgres','indicadors'));
			$table_consult->wrap = array ('nowrap','', '');
...
function calcularDades($start_time){
	...
	$select = "SELECT id,smoodle,spostgres, DATE_FORMAT(time_executed,'%d-%m-%Y %H:%i') AS time_executed ". 
	"FROM {$CFG->prefix}analytics_{$ADM->entorn_actual->id}_sessions WHERE time_executed >= STR_TO_DATE('00:00:00 ".$start_time->any."-".$start_time->mes."-".$start_time->dia."', '%H:%i:%S %Y-%m-%d') AND time_executed < STR_TO_DATE('00:00:00 ".$start_time->any."-".$start_time->mes."-".(($start_time->dia)+1)."', '%H:%i:%S %Y-%m-%d') ORDER BY time_executed ASC";
	//echo $select;
