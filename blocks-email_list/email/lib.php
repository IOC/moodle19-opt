...
function email_printblocks($userid, $courseid, $printsearchblock=true) {
	...
	// Get my course
	$mycourses = get_my_courses($USER->id, NULL, 'id, fullname, visible', false, $CFG->email_max_number_courses); // Limit
...
function email_is_enabled_email_list( $courseid ) {
    ...
    if ($emaillist = get_record('block', 'name', 'email_list') ) {
        ...
        if (!empty($block)) {
        ...
        } else if (record_exists('block_pinned', 'blockid', $emaillist->id, 'pagetype', 'course-view')) {
            ...
            } else if (record_exists('block_pinned', 'blockid', $emaillist->id, 'pagetype', 'my-index')) {
                return has_capability('moodle/block:view', get_context_instance(CONTEXT_SYSTEM));
            }
        }
    }
