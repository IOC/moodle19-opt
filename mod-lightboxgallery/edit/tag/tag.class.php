...
class edit_tag extends edit_base {
    ...
    function output() {
        ...
        $tags = get_records_select_menu('lightboxgallery_image_meta', $this->sql_select(), 'description', 'id,description');
        ...
        if (isset($info['APP13'])) {
            ...
            if (isset($iptc['2#025'])) {
                ...
                foreach ($iptc['2#025'] as $tag) {
                    ...
                    $iptcform .= '<label ' . ($exists ? 'class="tag-exists"' : '') . '><input type="checkbox" name="iptctags[]" value="' . urlencode($tag) . '" />' . s($tag) . '</label><br />';
                }
        ...
        if ($tags) {
            ...
            foreach ($tags as $tagid => $tagname) {
                $deleteform .= '<label><input type="checkbox" name="deletetags[]" value="' . $tagid . '" />' . s($tagname) . '</label><br />';
            }
